...
conda activate /projects/lotterhos/TPC_evol_SLiM/tpc_evo_slim
...
echo "csv file path is " $CSV_FILE
...
# Thus, if generation length depends on teperature, 
# we will find average generation length from one of the log files
# and use it for analytical prediction
if [[ ${GEN_LEN_DEPENDS_ON_TEMP} == 'T' ]]; then
	echo "generation length depends on temperature. Use a SLiM's log file to calculate average generation length."
	AVG_GEN_LEN=$(python find_avg_gen_len.py ${OUTDIR} ${OUTNAME})
else
	AVG_GEN_LEN=$FIXED_GEN_LEN
fi
