...
# Get CSV file from command line argument (already contains full path from submit script)
CSV_FILE="$1"
...
IFS=',' read -r seed RUNTIME_IF_NO_EXTERNAL_TEMP_DATA BURNIN\
 LOGINTERVAL N_POP MU QTN_var RECOMBINATION_RATE RECOVERY GEN_LEN_DEPENDS_ON_TEMP\
...
with seed=${seed} RUNTIME_IF_NO_EXTERNAL_TEMP_DATA=${RUNTIME_IF_NO_EXTERNAL_TEMP_DATA} BURNIN=${BURNIN} \n \
LOGINTERVAL=${LOGINTERVAL} N_POP=${N_POP} MU=${MU} QTN_var=${QTN_var} \n \
...
-d QTN_var=${QTN_var} -d RECOMBINATION_RATE=${RECOMBINATION_RATE} -d RECOVERY=${RECOVERY} \
